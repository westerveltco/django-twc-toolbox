...
1. Create a new git branch off of `main` for the release.
   ...
   Prefer the convention `release-<version>`, where `<version>` is the next incremental version number (e.g. `release-v0.2.1` for version 0.2.1).
...
6. Draft a [new release](https://github.com/westerveltco/django-twc-toolbox/releases/new) on GitHub.

   Use the version number with a leading `v` as the tag name (e.g. `v0.2.1`).
